...
✅ **LLM 选型确定：OpenAI API**
- 模型：`gpt-3.5-turbo`（意图分类 + 响应生成），`text-embedding-3-small`（向量化）
- 理由：7天时限下优先降低复杂度，API Key 通过 `.env` 管理，无需本地 GPU
- Docker Compose 无需额外 LLM 服务容器
...
| 意图分类 | OpenAI API zero-shot（gpt-3.5-turbo） |
