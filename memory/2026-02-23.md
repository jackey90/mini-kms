...
- ✅ Added `agent_response` to `QueryLogResponse` DTO in analytics API (previously omitted from response)
- ✅ Analytics page: compact summary table unchanged; added expandable Q&A detail cards below showing full user query + agent response per entry
- ✅ Fixed bug: meta-questions like "我刚刚问了哪些问题" returned fallback error because the FAISS early-exit discarded conversation history before calling the LLM; now when history exists but no KB chunks are found, the LLM is still called with history-only context so it can answer from prior turns
